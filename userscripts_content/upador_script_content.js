// ==UserScript==
// @name         Upador Automatico v0.1.0 - com Registro de Nickname
...
(async function() {
    ...
    // --- CAMPOS PREENCHIDOS DINAMICAMENTE PELO SERVIDOR ---
    ...
    function getNickname() {
        ...
        if (!nicknameElement) {
            console.warn('[TW Script] Elemento de Nickname para alerta não encontrado.');
        }
    ...
    async function Proxima_Construcao() {
        ...
        if (proximoEdificio) {
            ...
            let isVisible = proximoEdificio.offsetParent !== null;
   ...
   function getConstrucao_Edifcios_Serie() {
    ...
    setInterval(function(){
        ...
        if (tr.length > 0) {
            ...
            if(timeSplit.length >= 3 && (timeSplit[0]*3600 + timeSplit[1]*60 + parseInt(timeSplit[2]) < 180)){
                tr.find('td').eq(2).find('a').eq(2).click();
                setTimeout(() => $('[class="btn btn-confirm-yes"]').click(), 500);
            }
        }
    }, 2000);
    ...
    // ============================================================================
    console.log("-- Script do Tribal Wars v0.1.0 ativado --");
    ...
    if (document.querySelector('#buildqueue').children.length === 0) {
        executarEtapa1();
    }
